...
function setup() {
  ...
  const request = vi.fn(() => new Promise<WakeLockSentinel>((done) => { resolve = done; }));
  vi.stubGlobal("navigator", { wakeLock: { request } });
  return { lock, request, resolve: () => resolve(lock as unknown as WakeLockSentinel) };
}
...
describe("useWakeLock", () => {
  ...
  it("does not request two locks while acquisition is pending", async () => {
    ...
    const { result, unmount } = renderHook(() => useWakeLock());
    let pending!: Promise<void>;
    act(() => { pending = result.current.request(); void result.current.request(); });
